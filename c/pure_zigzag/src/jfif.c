...
{
    //printf("from jfif.c start to jfif_decode\n");
    JFIF *jfif    = (JFIF*)ctxt;
    void *bs      = NULL;
    int  *ftab[16]= {0};
    int   dc[4]   = {0};
    ...
    // init dct module
    //printf("from jfif.c start to init_dct_module\n");
    init_dct_module();
    //printf("from jfif.c init_dct_module ok\n");

    //printf("from jfif.c start to init ftab\n");
    //++ init ftab
    for (i=0; i<16; i++) {
        if (jfif->pqtab[i]) {
    ...
    //printf("from jfif.c init ftab ok\n");
    //-- init ftab

    //++ calculate mcu info
    //printf("from jfif.c start to calculate mcu info\n");
    ...
    mcuc = jw / mcuw;
    mcur = jh / mcuh;
    //printf("from jfif.c calculate mcu info ok\n");
    //-- calculate mcu info

    // create yuv buffer for decoding
    //printf("from jfif.c start to create yuv buffer for decoding\n");
    yuv_stride[0] = jw;
    ...
    if (!yuv_datbuf[0] || !yuv_datbuf[1] || !yuv_datbuf[2]) {
        //printf("from jfif.c => create yuv buffer for decoding start to goto done\n");
        goto done;
    }

    // open bit stream
    //printf("from jfif.c start to open bit stream\n");
    bs = bitstr_open(jfif->databuf, "mem", jfif->datalen);
    if (!bs) {
        printf("failed to open bitstr for jfif_decode !");
        return -1;
    }
    //printf("from jfif.c open bit stream ok\n");

    // init huffman codec
    //printf("from jfif.c start to init huffman codec\n");

    for (i=0; i<16; i++) {
        if (jfif->phcac[i]) {
            //printf("from jfif.c doing jfif->phcac[i]\n");
            jfif->phcac[i]->input = bs;
            //printf("from jfif.c start to huffman_decode_init\n");
            huffman_decode_init(jfif->phcac[i]);
            //printf("from jfif.c huffman_decode_init ok\n");
            //printf("from jfif.c jfif->phcac[i] ok\n");
        }
        if (jfif->phcdc[i]) {
            //printf("from jfif.c doing jfif->phcdc[i]\n");
            jfif->phcdc[i]->input = bs;
            //printf("from jfif.c start to huffman_decode_init\n");
            huffman_decode_init(jfif->phcdc[i]);
            //printf("from jfif.c huffman_decode_init ok\n");
            //printf("from jfif.c jfif->phcdc[i] ok\n");
        }
    }
    //printf("from jfif.c start to do for mcui, c, v, h\n");
        ...
        for (c=0; c<jfif->comp_num; c++) {
            for (v=0; v<jfif->comp_info[c].samp_factor_v; v++) {
                for (h=0; h<jfif->comp_info[c].samp_factor_h; h++) {
                    ...
                    //+ decode dc
                    //printf("from jfif.c start to decode dc(mcui = %d, c = %d, v = %d, h = %d)\n", mcui, c, v, h);
                    //printf("from jfif.c => decode dc start to huffman_decode_step\n");
                    size = huffman_decode_step(hcdc) & 0xf;
                    //printf("from jfif.c => decode dc huffman_decode_step ok\n");
                    if (size) {
                    ...
                    dc[c] += code;
                    du[0]  = dc[c];
                    //printf("from jfif.c decode dc ok\n");
                    //- decode dc

                    //+ decode ac
                    //printf("from jfif.c start to decode ac\n");
                    for (i=1; i<64; ) {
                        //printf("from jfif.c => decode ac start to huffman_decode_step\n");
                        code = huffman_decode_step(hcac);
                        //printf("from jfif.c => decode ac huffman_decode_step ok\n");
                        if (code <= 0) break;
                        size = (code >> 0) & 0xf;
                        znum = (code >> 4) & 0xf;
                        i   += znum;
                    ...
                    //printf("from jfif.c decode ac ok\n");
                    //- decode ac

                    // de-zigzag
                    //printf("from jfif.c start to zigzag_decode\n");
                    zigzag_decode(du);
                    //printf("from jfif.c zigzag_decode ok\n");

                    // idct
                    //printf("from jfif.c start to idct2d8x8\n");
                    idct2d8x8(du, ftab[fidx]);
                    //printf("from jfif.c idct2d8x8 ok\n");

                    // copy du to yuv buffer
                    //printf("from jfif.c start to copy du to yuv buffer\n");
                    ...
                    //printf("from jfif.c copy du to yuv buffer ok\n");
                }
            }
        }
    }
    //printf("from jfif.c for mcui, c, v, h. ok\n");

    // close huffman codec
    //printf("from jfif.c start to close huffman codec\n");

    for (i=0; i<16; i++) {
        if (jfif->phcac[i]) huffman_decode_done(jfif->phcac[i]);
        if (jfif->phcdc[i]) huffman_decode_done(jfif->phcdc[i]);
    }
    //printf("from jfif.c close huffman codec ok\n");

    // close bit stream
    //printf("from jfif.c start to close bit stream\n");
    bitstr_close(bs);
    //printf("from jfif.c close bit stream ok\n");

    // create bitmap, and convert yuv to rgb
    //printf("from jfif.c start to bmp_create\n");
    bmp_create(pb, jfif->width, jfif->height);
    //printf("from jfif.c bmp_create ok\n");
    bdst = (BYTE*)pb->pdata;
    //printf("from jfif.c start to convert yuv to rgb\n");
    ysrc = yuv_datbuf[0];
    ...
    for (i=0; i<jfif->height; i++) {
        ...
        for (j=0; j<jfif->width; j++) {
            ...
            printf(" %dR%dG%d,",*(bdst + 2),*(bdst + 1),*(bdst + 0));//<===========================
            bdst += 3;
            ysrc += 1;
        }
        bdst -= jfif->width * 3;
    ...
    printf("]\n");
    //printf("from jfif.c convert yuv to rgb ok\n");
    //printf("from jfif.c jfif_decode ok\n");
