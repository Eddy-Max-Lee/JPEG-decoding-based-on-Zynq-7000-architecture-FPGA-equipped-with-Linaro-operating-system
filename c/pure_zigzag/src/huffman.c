...
{
    //printf("from huffman.c start to huffman_decode_init\n");
    int i;
    ...
    printf("\n\n");
#endif
    //printf("from huffman.c huffman_decode_init ok\n");
}
...
{
    //printf("from huffman.c start to huffman_decode_step\n");
    int bit;
    int code = 0;
    int len  = 0;
    int idx  = 0;
    ...
    if (!phc || !phc->input)
    {
        //printf("from huffman.c return EOF\n");
        return EOF;
    }
...
//  printf("get code:%c len:%d, idx:%d\n\n", phc->huftab[idx], len, idx);
    //printf("from huffman.c huffman_decode_step ok\n");
