...
{
    //printf("from dct.c start to init_dct_module\n");
    int   i, j;
    float factor[64];
    ...
    // fdct factor
    //printf("from dct.c start to fdct factor\n");
    for (i=0; i<DCTSIZE; i++) {
        for (j=0; j<DCTSIZE; j++) {
    ...
    //printf("from dct.c fdct factor ok\n");
    
    //printf("from dct.c start to idct factor\n");
    // idct factor
    for (i=0; i<DCTSIZE; i++) {
        for (j=0; j<DCTSIZE; j++) {
            factor[i * 8 + j] = 1.0f * (AAN_DCT_FACTOR[i] * AAN_DCT_FACTOR[j] / 8);
    ...
    //printf("from dct.c idct factor ok\n");
    //printf("from dct.c init_dct_module ok\n");
}
...
{
    //add IP into code
    int dct_fd = open("/dev/dct", O_RDWR);
    //int dct_value;
    if (dct_fd < 0)
    {
        printf("[ERROR] can't open dct ip");
    }
    //printf("from dct.c start to idct2d8x8\n");
    int  tmp0,  tmp1,  tmp2,  tmp3;
    int  tmp4,  tmp5,  tmp6,  tmp7;
    int  tmp10, tmp11, tmp12, tmp13;
    ...
    /* Pass 1: process rows. */
    //printf("from dct.c start to Pass 1\n");
    dataptr = data;
    for (ctr=0; ctr<DCTSIZE; ctr++)
    {
        ...
        //part control = 0, fpga end
        tmp12 *= FLOAT2FIX(1.414213562f); tmp12 >>= FIXQ;
        tmp12 -= tmp13;

        //part control = 0, fpga start
        write(dct_fd, &tmp10, 0);
        write(dct_fd, &tmp11, 1);
        write(dct_fd, &tmp13, 2);
        write(dct_fd, &tmp12, 3);
        read(dct_fd, &tmp0, 0);
        read(dct_fd, &tmp3, 1);
        read(dct_fd, &tmp1, 2);
        read(dct_fd, &tmp2, 3);
        ...
        //tmp11  = z11 - z13;   /* 2 * c4 */
        //need input two empty variables to fit the fpga function
        //part control = 0, fpga end
        ...
        write(dct_fd, &tmp7, 2);
        write(dct_fd, &tmp6, 3);
        read(dct_fd, &dataptr[0], 0);
        read(dct_fd, &dataptr[7], 1);
        read(dct_fd, &dataptr[1], 2);
        read(dct_fd, &dataptr[6], 3);
        /*dataptr[0] = tmp0 + tmp7;
        dataptr[7] = tmp0 - tmp7;
        dataptr[1] = tmp1 + tmp6;
        dataptr[6] = tmp1 - tmp6;*/
        ...
        write(dct_fd, &tmp5, 2);
        write(dct_fd, &tmp4, 3);
        read(dct_fd, &dataptr[2], 0);
        read(dct_fd, &dataptr[5], 1);
        read(dct_fd, &dataptr[4], 2);
        read(dct_fd, &dataptr[3], 3);
        //dataptr[2] = tmp2 + tmp5;
        //dataptr[5] = tmp2 - tmp5;
        //dataptr[4] = tmp3 + tmp4;
        //dataptr[3] = tmp3 - tmp4;
    ...
    //printf("from dct.c Pass 1 ok\n");

    /* Pass 2: process columns. */
    //printf("from dct.c start to Pass 2\n");
    dataptr = data;
    for (ctr=0; ctr<DCTSIZE; ctr++)
    {
        /* Even part */
        ...
        write(dct_fd, &tmp7, 2);
        write(dct_fd, &tmp6, 3);
        read(dct_fd, &dataptr[DCTSIZE * 0], 0);
        read(dct_fd, &dataptr[DCTSIZE * 7], 1);
        read(dct_fd, &dataptr[DCTSIZE * 1], 2);
        read(dct_fd, &dataptr[DCTSIZE * 6], 3);
        ...
        write(dct_fd, &tmp5, 2);
        write(dct_fd, &tmp4, 3);
        read(dct_fd, &dataptr[DCTSIZE * 2], 0);
        read(dct_fd, &dataptr[DCTSIZE * 5], 1);
        read(dct_fd, &dataptr[DCTSIZE * 4], 2);
        read(dct_fd, &dataptr[DCTSIZE * 3], 3);
