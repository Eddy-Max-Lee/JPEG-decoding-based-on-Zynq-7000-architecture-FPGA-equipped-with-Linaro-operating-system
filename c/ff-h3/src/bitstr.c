...
{
    //add IP into code
    int bitstr_getb_fd = open("/dev/bitstr_getb", O_RDWR);
    //int dct_value;
    if (bitstr_getb_fd < 0)
    {
    ...
    //fpga start
    write(bitstr_getb_fd, &(context->bitbuf), 0);
    write(bitstr_getb_fd, &(context->bitnum), 1);
    read(bitstr_getb_fd, &bit, 0);
    read(bitstr_getb_fd, &(context->bitbuf), 1);
    read(bitstr_getb_fd, &(context->bitnum), 2);
    //bit = (context->bitbuf >> 7) & (1 << 0);
    //context->bitbuf <<= 1;
    //context->bitnum--;
    //fpga end
    close(bitstr_getb_fd);
    return bit;
}
