...
MODULE_LICENSE("GPL");
#define DEVICE_NAME "zz_4"
#define MYGPIO_PHY_ADDR 0x43C20000	// XPAR_MYGPIO_0_S00_AXI_BASEADDR -> 0x43C00000
#define MYGPIO_REG_NUM 64			// MYGPIO register number
...
static int call_zz_open(struct inode *inode , struct file *filp)
{
	return 0;
}

/* "release" is the function to release device */
static int call_zz_release(struct inode *inode , struct file *filp)
{
	return 0;
}

static ssize_t call_zz_read(struct file *fp, char *buffer, size_t reg, loff_t *offset)
{
	unsigned int val = 0;
	...
	{
		printk(KERN_ALERT"zz : [ERROR] Read Wrong register number !\n");
	}
...
static ssize_t call_zz_write(struct file *fp, const char *buffer, size_t reg, loff_t *offset)
{
	unsigned int val = 0;
...
{
	printk(KERN_ALERT"Hello world zz driver\n");
